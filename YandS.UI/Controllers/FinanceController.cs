...
namespace YandS.UI.Controllers
{
    ...
    public class FinanceController : Controller
    {
        ...
        public ActionResult AjaxIndexDataPV(string DataFor)
        {
            ...
            string[] ArrDataFor = { "PENDING", "REFAPPROVED", "NRAPPROVED", "NEWPDC", "REJECT", "PV-TABLE", "CASE-TABLE", "TRANS-TABLE", "INV-TABLE" };
        ...
        {
            ViewBag.Debit_Account = new SelectList(Helper.GetBankList(), "Mst_Value", "Mst_Desc");
            ViewBag.Payment_Head = new SelectList(db.MasterSetup.Where(m => m.MstParentId == (int)MASTER_S.PaymentHeads && m.Remarks == "N" && m.Active_Flag == true).OrderBy(o => o.DisplaySeq), "Mst_Value", "Mst_Desc");
            ViewBag.Payment_To = new SelectList(Helper.GetListForPayTo(), "Mst_Value", "Mst_Desc");
        ...
        public ActionResult PayVoucherApproveRefNonRef(PayVoucher payVoucher, HttpPostedFileBase upload)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //return RedirectToAction("PayVoucherIndexRefNonRef", new RouteValueDictionary(new { id = payVoucher.VoucherType, mode = "SA" }));
            ...
            //return RedirectToAction("PayVoucherIndexRefNonRef", new RouteValueDictionary(new { id = payVoucher.VoucherType, mode = "EA", payVoucher }));
        }

        [HttpGet]
        public ActionResult GetTab(string Mode, string PartialViewName, int Voucher_No, PayVoucher modal = null)
        {
            if (PartialViewName == "_PVTable" || PartialViewName == "_CaseTable")
            {
        ...
        public List<PayVoucher> GetPayVoucherList(string LocationName, int VoucherNo = 0)
        {
            ...
            /*
            if (!string.IsNullOrEmpty(LocationName))
                ResultList = db.PayVouchers.Where(w => w.PV_No.Substring(0, 3).ToUpper() == LocationName.Substring(0, 3).ToUpper()).ToList();
            else
            ...
            var ReturnResult = (from res in ResultList
                                ...
                                join PayTo in Helper.GetListForPayTo()
                                ...
                                join CourtType in db.Set<MasterSetups>().Where(m => m.MstParentId == (int)MASTER_S.CaseLevel)
                                ...
                                join TrType in db.Set<MasterSetups>().Where(m => m.MstParentId == (int)MASTER_S.PVTransType)
                                on res.TransTypeCode equals TrType.Mst_Value
                                join TrReason in db.Set<MasterSetups>().Where(m => m.MstParentId == (int)MASTER_S.PVTransReason)
                                on res.TransReasonCode equals TrReason.Mst_Value
                                select new PayVoucher
                                {
                                    ...
                                    DebitAccountName = string.Format(@"{0} : {1}", bank_mas.Mst_Desc, BankAccList.AccountNumber),
                                    CreditAccountName = res.Credit_Account == 0 ? null : string.Format(@"{0} : {1}", bank_masCR.Mst_Desc, BankAccListCR.AccountNumber),
                                    ...
                                    PV_No = res.PV_No,
                                    TransTypeCode = res.TransTypeCode,
                                    TransTypeName = TrType.Mst_Desc,
                                    TransReasonCode = res.TransReasonCode,
                                    TransReasonName = TrReason.Mst_Desc,
                                    CaseId = res.CaseId,
                                    CaseInvoices = res.CaseInvoices
                                }).ToList();
            */
            return ResultList;
        }
